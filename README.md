...
2. Download the latest release produced by GitHub Actions:
    ```
    wget https://github.com/xenago/libnss_shim/releases/download/1.0.1/libnss_shim_1.0.1_amd64.deb
    ```

3. Install it directly with `dpkg` or through `apt`:
    ```
    sudo dpkg -i libnss_shim_1.0.1_amd64.deb
    ```
   or
    ```
    sudo apt install ./libnss_shim_1.0.1_amd64.deb
    ```
